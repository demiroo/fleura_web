...
import { useCartModal } from "./cart-modal-manager";
...
export default function CartModal() {
  ...
  useEffect(() => {
    // Only auto-open cart when quantity increases (item added)
    if (
      cart?.totalQuantity &&
      cart?.totalQuantity > (quantityRef.current || 0) &&
    ...
    // Always update the ref to track current quantity
    if (cart?.totalQuantity !== undefined) {
      quantityRef.current = cart?.totalQuantity;
    }
  }, [cart?.totalQuantity, isCartModalOpen, openCartModal]);
  ...
  return (
    <>
      ...
      <Transition show={isCartModalOpen}>
            ...
            <Dialog.Panel className="fixed bottom-0 right-0 top-0 flex h-full w-full flex-col border-l border-neutral-200 bg-white/80 p-6 text-black backdrop-blur-xl md:w-[390px] dark:border-neutral-700 dark:bg-black/80 dark:text-white">
              ...
              {!cart || cart.lines.length === 0 ? (
                ...
                <div className="flex h-full flex-col justify-between overflow-hidden p-1">
                  ...
                  <form action={redirectToCheckout as any}>
                    <CheckoutButton />
                  </form>
                </div>
              )}
