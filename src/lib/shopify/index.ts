...
export async function logoutCustomer(customerAccessToken: string): Promise<{ success: boolean; errors: any[] }> {
  try {
    ...
    console.warn('Failed to logout via Shopify API:', error);
    // Return success=false but don't throw - logout should still work locally
    return {
      success: false,
...
export async function recoverCustomerPassword(email: string): Promise<{ errors: any[] }> {
  const res = await shopifyFetch<ShopifyCustomerPasswordResetOperation>({
